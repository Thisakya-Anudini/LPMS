...
const getAllowedEmployeeNumbers = () => {
  const configured = String(
    process.env.EMAIL_TEST_EMPLOYEE_NUMBERS || "",
  ).trim();
  const source = configured
    ? configured.split(",")
    : DEFAULT_ALLOWED_EMPLOYEE_NUMBERS;
  return new Set(source.map(normalizeEmployeeNumber).filter(Boolean));
};

const isEmailEnabled = () =>
  String(process.env.EMAIL_ENABLED || "true").toLowerCase() !== "false";
...
export const canSendEmailToEmployee = (employeeNumber) =>
  isEmailEnabled() &&
  getAllowedEmployeeNumbers().has(normalizeEmployeeNumber(employeeNumber));
...
const getSmtpConfig = () => ({
  ...
  startTls:
    String(process.env.SMTP_STARTTLS || "false").toLowerCase() === "true",
  rejectUnauthorized:
    String(
...
const getTestRecipientOverride = () =>
  String(process.env.EMAIL_TEST_REDIRECT_TO || "")
    .trim()
    .toLowerCase();
...
const createBoundary = () =>
  `lpms_${Date.now()}_${crypto.randomBytes(8).toString("hex")}`;
...
const createSmtpSession = (config) =>
  new Promise((resolve, reject) => {
...
const readResponse = (socket) =>
  new Promise((resolve, reject) => {
    ...
    const onClose = () =>
      settle(
        reject,
        new Error("SMTP connection closed before server response."),
      );
...
const writeCommand = async (socket, command, expectedCodes = []) => {
  ...
  if (expectedCodes.length > 0 && !expectedCodes.includes(response.code)) {
    throw new Error(
      `SMTP command failed (${response.code}): ${response.message}`,
    );
  }
  return response;
};
...
export const sendEmail = async ({ to, subject, html, text }) => {
  ...
  try {
    ...
    if (dataResponse.code !== 250) {
      throw new Error(
        `SMTP data transmission failed (${dataResponse.code}): ${dataResponse.message}`,
      );
    }
...
const safelySendNotificationEmail = async ({
  employeeNumber,
  to,
  subject,
  html,
...
export const sendLearningPathAssignedEmail = async ({
  employeeNumber,
  to,
  learnerName,
  learningPathTitle,
