...
export const renderCertificatePdf = async ({
  ...
  doc.rect(32, 32, pageWidth - 64, pageHeight - 64).lineWidth(1.5).stroke('#1d4ed8');

  doc.font('Helvetica-Bold').fontSize(28).fillColor('#0f172a').text(
    `${String(certificateTitle || '').toUpperCase()} CERTIFICATE`,
    leftMargin,
    70,
    {
      align: 'center',
  ...
  doc.moveDown(0.35);
  doc.font('Helvetica-Bold').fontSize(24).fillColor('#0f172a').text(learnerName || '-', {
    align: 'center',
    width: contentWidth
  });
  doc.moveDown(0.15);
  ...
  doc.font('Helvetica-Bold').fontSize(11).fillColor('#0f172a').text('Courses', tableLeft + 8, tableTop + 7, {
    width: courseColWidth - 12
  });
  doc.text('Duration (hours)', tableLeft + courseColWidth + 8, tableTop + 7, {
  ...
  if (normalizedCourses.length === 0) {
    ...
    for (const course of normalizedCourses) {
      ...
      doc.font('Helvetica').fontSize(11).fillColor('#334155').text(course.title, tableLeft + 8, currentRowTop + 6, {
        width: courseColWidth - 12
      });
      doc.text(course.duration, tableLeft + courseColWidth + 8, currentRowTop + 6, {
        width: durationColWidth - 12,
        align: 'right'
      });
      currentRowTop += rowHeight;
  ...
  doc.font('Helvetica').fontSize(11).fillColor('#334155').text(
    `Learning Path Duration: ${learningPathDuration || '-'}`,
    leftMargin,
    doc.y,
    {
      width: contentWidth
