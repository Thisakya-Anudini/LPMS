...
function calculateVisiblePages(page: number, totalPages: number) {
  if (totalPages <= 1) {
...
function LearnerSearchFilters({
  ...
  employeeNoSearchError: string;
  onEmployeeNoChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  nameSearch: string;
  ...
  return (
    <div className="border-b border-slate-100 bg-slate-50/70 px-6 py-5">
      <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
        <SlidersHorizontal className="h-3.5 w-3.5" /> Search & filters
      </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              aria-describedby={
                employeeNoSearchError
                  ? "learner-employee-no-search-error"
                  : undefined
              }
            />
          </div>
          {employeeNoSearchError && (
            <p
              id="learner-employee-no-search-error"
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              aria-describedby={
                nameSearchError ? "learner-name-search-error" : undefined
              }
            />
          </div>
          {nameSearchError && (
            <p
              id="learner-name-search-error"
              className="mt-1 text-xs font-medium text-red-600"
            >
              {nameSearchError}
            </p>
...
function LearnerTableRow({
  ...
  learner: LearnerRow;
  onOpenDetails: (learner: LearnerRow) => void;
}) {
  const percentComplete = learner.total_learning_paths
    ? Math.round(
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-3">
        ...
        {learner.grade_name && (
          <p className="mt-0.5 text-xs text-slate-400">{learner.grade_name}</p>
        )}
      </td>
      <td className="px-3 py-3">
        <div className="min-w-[150px]">
          <div className="mb-2 flex items-center justify-between text-xs">
            <span className="font-semibold text-slate-700">
              {learner.completed_learning_paths} of{" "}
              {learner.total_learning_paths} completed
            </span>
...
function LearnerPaginationControls({
  loading,
  page,
  totalPages,
  canGoPrevious,
...
export function AdminLearnersPage() {
  ...
  const handleEmployeeNoSearchChange = (
  ...
  const hasActiveFilters = Boolean(
    employeeNoSearch || nameSearch || designationFilter !== "ALL",
  );
  const clearFilters = () => {
    setEmployeeNoSearch("");
    setNameSearch("");
  ...
  const visiblePages = useMemo(
    () => calculateVisiblePages(pagination.page, pagination.totalPages),
    ...
    <div className="space-y-6 pb-6">
        ...
        <div className="relative flex flex-col gap-4 px-5 py-6 md:px-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full flex-col gap-1 rounded-xl border border-primary-100 bg-primary-50/50 px-5 py-4 sm:w-auto sm:min-w-[240px] transition-all duration-200 hover:border-primary-200 hover:bg-primary-50/90 hover:shadow-xs">
            ...
            <p className="mt-1 text-xs font-medium text-primary-700">
              {loading
                ? "Refreshing learners..."
                : `${pagination.total.toLocaleString()} total active learners`}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="shadow-sm" bodyClassName="p-0">
          ...
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Use a name, employee number, or designation to narrow the
                directory.
              </p>
            </div>
            {hasActiveFilters && (
        ...
        <LearnerSearchFilters
          employeeNoSearch={employeeNoSearch}
          employeeNoSearchError={employeeNoSearchError}
          onEmployeeNoChange={handleEmployeeNoSearchChange}
          nameSearch={nameSearch}
          nameSearchError={nameSearchError}
          onNameChange={handleNameSearchChange}
          designationFilter={designationFilter}
          designationOptions={designationSelectOptions}
                ...
                {loading ? (
                  ...
                  learners.map((learner) => (
                    <LearnerTableRow
                      ...
                      learner={learner}
                      onOpenDetails={openLearnerDetails}
                    />
                  ))
                )}
              </tbody>
